...
export async function processImageWithGemini(imageBase64: string): Promise<OcrResult> {
  try {
    // Try gemini-pro-vision for vision tasks, fallback to gemini-1.5-pro if needed
...
3. **Weight**: CRITICAL - Find the total weight from the BOTTOM RIGHT CELL of the table:
   ...
   STEP 2: Find the very last row of the data table - this is the SUMMARY/TOTAL row at the bottom
   STEP 3: In that bottom row, look at the RIGHTMOST CELL - that's where the weight is located
   STEP 4: Extract the number from that bottom right cell
   ...
   IMPORTANT: The weight is in the BOTTOM RIGHT CELL of the table, not in a specific column position
...
4. **Supplier**: CRITICAL - Extract the supplier name from the data table:
   ...
   STEP 2: Locate the "SUPPLIER" column in the header - count from the LEFT: the leftmost column is #1, next is #2, SUPPLIER is the THIRD column from the LEFT (#3)
   STEP 3: Now look at ALL the DATA ROWS (skip the header row and skip the summary row at the bottom)
   STEP 4: For each data row, read the text in the THIRD column from the LEFT (the SUPPLIER column position)
   ...
   CRITICAL: Do NOT return empty string - if you see ANY supplier names in the table, return the most common one. Even if you're not 100% sure, return the best match you can find. The supplier is in the THIRD COLUMN FROM THE LEFT.
