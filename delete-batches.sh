#!/bin/bash
# Script to delete all batches from MongoDB Atlas (production database)

docker exec metcon-workflow-app-backend-1 node -e "
const mongoose = require('mongoose');
const { Batch } = require('./dist/models/Batch.js');
